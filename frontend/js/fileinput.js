(function (factory) {
        ...
        init: function (obj) {
            ...
            previewCache.data[id] = {
                ...
                actions: function (showUpload, showDelete, showZoom, showDrag, disabled, url, key) {
                    return obj._renderFileActions(showUpload, showDelete, showZoom, showDrag, disabled, url, key, true, obj.initialPreviewConfig[key].caption, obj.initialPreviewConfig[key].group);
                }
            };
        },
        fetch: function (id) {
    ...
    tProgress = '<div class="progress">\n' +
        ...
        '     </div>\n' +
        '</div>';
	tSize = ' <br><samp></samp>'; //tSize = ' <br><samp>({sizeText})</samp>';
    ...
    FileInput.prototype = {
        ...
        _parseError: function (jqXHR, errorThrown, fileName) {
			...
			try {
				text = jqXHR.responseJSON !== undefined && jqXHR.responseJSON.error !== undefined ?
					jqXHR.responseJSON.de_DE : jqXHR.responseJSON.de_DE;
			} catch (err){
				text = "Leider ist ein Fehler beim Hochladen aufgetreten. Bitte versuchen Sie es später erneut oder kontaktieren Sie uns direkt.";
			}
            ...
            self.$preview.find('.kv-file-remove').each(function () {
                ...
                settings = $.extend(true, {}, {
                    ...
                    error: function (jqXHR, textStatus, errorThrown) {
						//console.log("ERROR (fileinput2)" + JSON.stringify(jqXHR)); // TODO is shown
        ...
        _ajaxSubmit: function (fnBefore, fnSuccess, fnComplete, fnError, previewId, index, file, filename) {
			...
			reader.onloadend = function () {
				var wordArray = CryptoJS.lib.WordArray.create(CryptoJS.enc.Latin1.parse(reader.result));
			...
			reader.onloadend = function () {
			   ...
			   //console.log(CryptoJS.SHA256(CryptoJS.enc.Latin1.parse(reader.result)).toString());
			};
        ...
        _uploadSingle: function (i, files, allFiles) {
            ...
            fnSuccess = function (data, textStatus, jqXHR) {
				...
				/*
				// Idear of moving back to list after uploading complete
				console.log(_imagesTitles.length + ' // ' + params.index);
				if (false) {
            ...
            fnError = function (jqXHR, textStatus, errorThrown) {
				console.log("ERROR (fileinput) " + JSON.stringify(jqXHR));
            ...
            self._ajaxSubmit(fnBefore, fnSuccess, fnComplete, fnError, previewId, i, files[i], self.filenames[i]);
        },
        ...
        _initFileActions: function () {
            ...
            self.$preview.find('.kv-file-remove').each(function () {
                ...
                handler($el, 'click', function () {
					try { _imagesTitles.splice(_openImmo_json.anhaenge.anhang.length+id,1); } catch (e) { _imagesTitles.splice(id,1) }
        ...
        _getFileName: function (file) {
			_imageName = file && file.name ? this.slug(file.name) : '';
            return _imageName;
        },
        ...
        _renderFileActions: function (showUpload, showDelete, showZoom, showDrag, disabled, url, key, isInit, titlename, group) {
			...
			var choosedGroup = '<select class="form-control" id="imagegroup' + _imagesTitles.length + '">\
									<option value="0">Keine Angabe</option>\
									<option value="TITELBILD"' + ((group == "TITELBILD") ?"selected" : "" ) + '>Titelbild</option>\
									<option value="INNENANSICHTEN"' + ((group == "INNENANSICHTEN") ?"selected" : "" ) + '>Innenansicht</option>\
									<option value="AUSSENANSICHTEN"' + ((group == "AUSSENANSICHTEN") ?"selected" : "" ) + '>Außenansicht</option>\
									<option value="GRUNDRISS"' + ((group == "GRUNDRISS") ?"selected" : "" ) + '>Grundriss</option>\
									<option value="KARTEN_LAGEPLAN"' + ((group == "KARTEN_LAGEPLAN") ?"selected" : "" ) + '>Karte / Lageplan</option>\
									<option value="ANBIETERLOGO"' + ((group == "ANBIETERLOGO") ?"selected" : "" ) + '>Logo</option>\
									<option value="BILD"' + ((group == "BILD") ?"selected" : "" ) + '>Bild</option>\
									<option value="DOKUMENTE"' + ((group == "DOKUMENTE") ?"selected" : "" ) + '>Dokument</option>\
									<option value="QRCODE"' + ((group == "QRCODE") ?"selected" : "" ) + '>QR-Code</option>\
									<option value="FILM"' + ((group == "FILM") ?"selected" : "" ) + '>Film</option>\
									<option value="EPASS-SKALA"' + ((group == "EPASS-SKALA") ?"selected" : "" ) + '>Energiepass-Skala</option>\
								</select>';
			var title = '<input type="text" class="form-control" id="imagetitle' + _imagesTitles.length + '" placeholder="Titel eingeben" value="' + ((titlename != null) ?titlename :_imageName.substring(0, _imageName.indexOf("."))) + '">';
			_imagesTitles.push(_imageName.substring(0, _imageName.indexOf(".")));
			_imageName = '';
            return title + choosedGroup + template.replace(/\{delete}/g, btnDelete)
...
function getID(index) {
	try {
		return _openImmo_json.anhaenge.anhang[index].id;
	} catch (e) { }
	return '';
}